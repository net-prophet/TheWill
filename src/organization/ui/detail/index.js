...
class AddBoardMemberForm extends React.Component {
  ...
  onSubmit(e) {
    e.preventDefault();
    this.props.contract.addBoardMember(this.state.address.trim(), {from: window.web3.eth.accounts[0]}, (err, result) => {
...
class Delegates extends React.Component {
  ...
  componentWillMount() {
    ...
    this.props.contract.getDelegate(window.web3.eth.accounts[0], (err, delegate) => {
  ...
  onSubmit(e) {
    e.preventDefault();
    this.props.contract.setDelegate(this.state.address.trim(), {from: window.web3.eth.accounts[0]}, (err, result) => {
      window.location.reload();
    })
    return false;
  }
  ...
  componentWillMount() {
    if (this.props.params.address) {
      ...
      this.contract.getOrgDetails((err, result) => {
        ...
        this.contract.isBoardMember(window.web3.eth.accounts[0], (err, result) =>
          this.setState({
            imOnTheBoard: true
          })
        );
